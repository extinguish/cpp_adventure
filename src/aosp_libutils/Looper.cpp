...
// Maximum number of file descriptors for which to retrieve poll events each iteration.
    ...
    sp<Looper> Looper::prepare(int opts) {
        ...
        if (looper->getAllowNonCallbacks() != allowNonCallbacks) {
            LOGE("Looper already prepared for this thread with a different value for the "
    ...
    void Looper::rebuildEpollLocked() {
        ...
        if (mEpollFd < 0) {
            LOGE("Could not create epoll instance: %s", strerror(errno));
        }
        ...
        if (result != 0) {
            LOGE("Could not add wake event fd to epoll instance: %s", strerror(errno));
        }
    ...
    int Looper::pollInner(int timeoutMillis) {
        ...
        if (eventCount < 0) {
            ...
            LOGE("Poll failed with an unexpected error: %s", strerror(errno));
            result = POLL_ERROR;
            goto Done;
        }
        ...
        for (int i = 0; i < eventCount; i++) {
            ...
            if (fd == mWakeEventFd.get()) {
                if (epollEvents & EPOLLIN) {
                    ...
                    LOGE("Ignoring unexpected epoll events 0x%x on wake event fd.", epollEvents);
                }
            } else {
                ...
                if (requestIndex >= 0) {
                    ...
                    LOGE("Ignoring unexpected epoll events 0x%x on fd %d that is "
    ...
    void Looper::wake() {
        ...
        if (nWrite != sizeof(uint64_t)) {
            if (errno != EAGAIN) {
                LOGE("Could not write wake signal to fd %d: %s", mWakeEventFd.get(),
                                 strerror(errno));
            }
        }
    }
