...
// ---------------------------------------------------------------------------
    ...
    void RefBase::incStrong(const void *id) const {
        ...
        if (c > 0) {
            LOGE("incStrong() called on %p after last strong ref", refs);
        }
#if PRINT_REFS
...
#endif
        if (BAD_STRONG(c)) {
            LOGE("decStrong() called on %p too many times", refs);
        }
        if (c == 1) {
    ...
    void RefBase::forceIncStrong(const void *id) const {
        ...
        const int32_t c = refs->mStrong.fetch_add(1, std::memory_order_relaxed);
        if (c >= 0) {
            LOGE("forceIncStrong called on %p after ref count underflow", refs);
        }
#if PRINT_REFS
    ...
    void RefBase::weakref_type::incWeak(const void *id) {
        ...
        const int32_t c __unused = impl->mWeak.fetch_add(1,
                                                         std::memory_order_relaxed);
        if (c >= 0) {
            LOGE("incWeak called on %p after last weak ref", this);
        }
    }
    ...
    void RefBase::weakref_type::decWeak(const void *id) {
        ...
        if (BAD_WEAK(c)) {
            LOGE("decWeak called on %p too many times", this);
        }
        if (c != 1) return;
        ...
        if ((flags & OBJECT_LIFETIME_MASK) == OBJECT_LIFETIME_STRONG) {
            ...
            if (impl->mStrong.load(std::memory_order_relaxed)
                ...
                LOGE("RefBase: Object at %p lost last weak reference "
    ...
    bool RefBase::weakref_type::attemptIncStrong(const void *id) {
        ...
        if (curCount >= 0) {
            LOGE("attemptIncStrong called on %p after underflow", this);
        }
    ...
    bool RefBase::weakref_type::attemptIncWeak(const void *id) {
        ...
        if (curCount >= 0) {
            LOGE("attemptIncWeak called on %p after underflow", this);
        }
        while (curCount > 0) {
