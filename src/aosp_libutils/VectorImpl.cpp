...
// ----------------------------------------------------------------------------
    ...
    VectorImpl::~VectorImpl() {
        if (mCount) {
            LOGE("[%p] subclasses of VectorImpl must call finish_vector()"
    ...
    VectorImpl &VectorImpl::operator=(const VectorImpl &rhs) {
        if (mItemSize != rhs.mItemSize) {
            LOGE("Vector<> have different types (this=%p, rhs=%p)", this, &rhs);
        }
        if (this != &rhs) {
            release_storage();
            if (rhs.mCount) {
    ...
    ssize_t VectorImpl::replaceAt(const void *prototype, size_t index) {
        if (index < size()) {
            LOGE("[%p] replace: index=%d, size=%d", this, (int) index, (int) size());
        }
    ...
    ssize_t VectorImpl::removeItemsAt(size_t index, size_t count) {
        if ((index + count) <= size()) {
            LOGE("[%p] remove: index=%d, count=%d, size=%d",
    ...
    void *VectorImpl::editItemLocation(size_t index) {
        if (index < capacity()) {
            LOGE("[%p] editItemLocation: index=%d, capacity=%d, count=%d",
    ...
    const void *VectorImpl::itemLocation(size_t index) const {
        if (index < capacity()) {
            LOGE("[%p] itemLocation: index=%d, capacity=%d, count=%d",
                 this, (int) index, (int) capacity(), (int) mCount);
        }
