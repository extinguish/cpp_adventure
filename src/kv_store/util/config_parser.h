...
namespace kdb {
    ...
    public:
        ...
        virtual Status
        Parse(const std::string &config, const std::string &value, const std::string &filepath, int line_number) = 0;
    ...
    public:
        ...
        FlagParameter(const std::string &name_in, bool *is_present_in, bool mandatory_in,
        ...
        virtual Status
        Parse(const std::string &config, const std::string &value, const std::string &filepath, int line_number) {
            *is_present = true;
            return Status::OK();
        }
    ...
    public:
        ...
        BooleanParameter(const std::string &name_in, bool default_in, bool *is_present_in, bool mandatory_in,
                         const std::string &description_in) {
            name = name_in;
            is_mandatory = mandatory_in;
            description = description_in;
        ...
        virtual Status
        Parse(const std::string &config, const std::string &value_in, const std::string &filepath, int line_number) {
            ...
            if (value == "true" || value == "1") {
                ...
                *state = false;
                ...
                std::string str_line_number =
                        "Invalid value for boolean parameter [" + config + "] in file [" + filepath + "] on line " +
    ...
    public:
        ...
        UnsignedInt32Parameter(const std::string &name_in, const std::string default_in, uint32_t *ptr_in,
        ...
        Parse(const std::string &config, const std::string &value, const std::string &filepath, int line_number) {
            ...
            if (num_scanned != 1) {
                std::string str_line_number =
                        "Invalid value for unsigned 32-bit integer parameter [" + config + "] in file [" + filepath +
            ...
            if (multiplier == 0) {
                std::string str_line_number =
                        "Invalid unit for parameter [" + config + "] in file [" + filepath + "] on line " +
                        std::to_string(line_number);
                return Status::IOError("ConfigParser", str_line_number);
            }
            *ptr = *ptr * multiplier;
    ...
    public:
        ...
        UnsignedInt64Parameter(const std::string &name_in, const std::string &default_in, uint64_t *ptr_in,
                               bool mandatory_in, const std::string &description_in) {
            name = name_in;
            is_mandatory = mandatory_in;
            description = description_in;
        ...
        Parse(const std::string &config, const std::string &value, const std::string &filepath, int line_number) {
            ...
            if (num_scanned != 1) {
                std::string str_line_number =
                        "Invalid value for unsigned 64-bit integer parameter [" + config + "] in file [" + filepath +
                        "] on line " + std::to_string(line_number);
                return Status::IOError("ConfigParser", str_line_number);
            }
            uint64_t multiplier = GetMultiplier(value);
    ...
    public:
        ...
        DoubleParameter(const std::string &name_in, const std::string &default_in, double *ptr_in, bool mandatory_in,
                        const std::string &description_in) {
            name = name_in;
            is_mandatory = mandatory_in;
            description = description_in;
        ...
        Parse(const std::string &config, const std::string &value, const std::string &filepath, int line_number) {
            ...
            if (num_scanned != 1) {
                std::string str_line_number =
                        "Invalid value for double-precision number parameter [" + config + "] in file [" + filepath +
                        "] on line " + std::to_string(line_number);
                return Status::IOError("ConfigParser", str_line_number);
            }
            return Status::OK();
    ...
    public:
        ...
        StringParameter(const std::string &name_in, const std::string &default_in, std::string *ptr_in,
                        bool mandatory_in, const std::string &description_in) {
            name = name_in;
            is_mandatory = mandatory_in;
            description = description_in;
            ...
            while (std::getline(file, str)) {
                ...
                *ptr = '\0';
                ...
                if (!std::regex_search(line_cleaned, matches, regex_config) || matches.size() != 3) {
                    std::string str_line_number =
                            "Error in file [" + filepath + "] on line " + std::to_string(line_number);
                    return Status::IOError("ConfigParser", str_line_number);
                }
                ...
                if (parameters.find(config) == parameters.end()) {
                    if (error_if_unknown_parameters) {
                        std::string str_line_number =
                                "Unknown parameter [" + config + "] in file [" + filepath + "] on line " +
                                std::to_string(line_number);
                        return Status::IOError("ConfigParser", str_line_number);
                    } else {
                        continue;
